...
# Avoid macOS Desktop/Documents TCC blocks: keep bind mounts under $HOME.
MILVUS_DATA_DIR="${MILVUS_DATA_DIR:-$HOME/.milvus/rag_start}"

run_embed() {
    mkdir -p "${MILVUS_DATA_DIR}/volumes/milvus"

    cat << EOF > "${MILVUS_DATA_DIR}/embedEtcd.yaml"
...
EOF
    ...
    if [ ! -f "${MILVUS_DATA_DIR}/user.yaml" ]
    then
        cat << EOF > "${MILVUS_DATA_DIR}/user.yaml"
# Extra config to override default milvus.yaml
EOF
    fi

    echo "Milvus data dir: ${MILVUS_DATA_DIR}"
    ...
    docker run -d \
        ...
        -v "${MILVUS_DATA_DIR}/volumes/milvus":/var/lib/milvus \
